...
(
  ...
  CONSTRAINT payment_ukey
    UNIQUE (invoice_id, payment_id, sequence_id)
);
...
(
  ...
  CONSTRAINT adjustment_ukey
    UNIQUE (invoice_id, payment_id, adjustment_id, sequence_id)
);
...
(
  ...
  CONSTRAINT refund_ukey
    UNIQUE (invoice_id, payment_id, refund_id, sequence_id)
);
...
create or replace function rpt.is_cash_flow_contains_amount_by_types(data_with_cash_flows jsonb, source_account_type text, source_account_type_value text, destination_account_type text,
...
begin
  ...
  for cash_flow in select * from jsonb_array_elements(data_with_cash_flows -> 'cash_flows')
    loop
      if cash_flow @> concat('{"source":{"account_type":{"', source_account_type, '":"', source_account_type_value, '"}}}')::jsonb
        and cash_flow @> concat('{"destination":{"account_type":{"', destination_account_type, '":"', destination_account_type_value, '"}}}')::jsonb
...
create or replace function rpt.is_cash_flow_contains_amount_by_types(data_with_cash_flows jsonb, source_account_type text, destination_account_type text) returns boolean
  immutable
  parallel safe
  language plpgsql
as
  ...
  for cash_flow in select * from jsonb_array_elements(data_with_cash_flows -> 'cash_flows')
    loop
      if (((cash_flow ->> 'source')::jsonb ->> 'account_type')::jsonb ->> source_account_type is not null)
        and (((cash_flow ->> 'destination')::jsonb ->> 'account_type')::jsonb ->> destination_account_type is not null)
        and ((cash_flow ->> 'volume')::jsonb ->> 'amount' is not null) then
        return true;
      end if;
    end loop;
...
create or replace function rpt.get_cash_flow_value(data_with_cash_flows jsonb, source_account_type text, source_account_type_value text, destination_account_type text,
...
$$
begin
  if (rpt.is_cash_flow_contains_amount_by_types(data_with_cash_flows, source_account_type, source_account_type_value, destination_account_type, destination_account_type_value) is not true) then
  ...
  return (
    ...
    where cash_flows_array @> concat('{"source":{"account_type":{"', source_account_type, '":"', source_account_type_value, '"}}}')::jsonb
      and cash_flows_array @> concat('{"destination":{"account_type":{"', destination_account_type, '":"', destination_account_type_value, '"}}}')::jsonb
      and ((cash_flows_array ->> 'volume')::jsonb ->> 'amount' is not null)
  );
end;
$$;

create or replace function rpt.get_cash_flow_value(data_with_cash_flows jsonb, source_account_type text, destination_account_type text, default_value bigint) returns bigint
  immutable
  parallel safe
  language plpgsql
as
$$
begin
  if (rpt.is_cash_flow_contains_amount_by_types(data_with_cash_flows, source_account_type, destination_account_type) is not true) then
    return default_value;
  end if;
  return (
    select sum(cast(((cash_flows_array ->> 'volume')::jsonb ->> 'amount') as bigint))
    ...
    where (((cash_flows_array ->> 'source')::jsonb ->> 'account_type')::jsonb ->> source_account_type is not null)
      and (((cash_flows_array ->> 'destination')::jsonb ->> 'account_type')::jsonb ->> destination_account_type is not null)
      and ((cash_flows_array ->> 'volume')::jsonb ->> 'amount' is not null)
  );
end;
$$;
...
begin
  return rpt.get_cash_flow_value(data_with_cash_flows, source_account_type, source_account_type_value, destination_account_type, destination_account_type_value, default_value);
end;
$$;
