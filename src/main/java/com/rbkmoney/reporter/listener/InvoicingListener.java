...
public class InvoicingListener {
    ...
    public void listen(List<ConsumerRecord<String, SinkEvent>> messages, Acknowledgment ack) {
        ...
        Map<BatchKeyGenerator, Map<UniqueBatchKey, List<MapperPayload>>> mapperPayloadsByUniqueKeyByType = getMapperPayloadsByUniqueKeyByType(messages);

        List<Query> saveEventQueries = new ArrayList<>();

        Map<UniqueBatchKey, PartyData> partyDataCache = new HashMap<>();
        Map<UniqueBatchKey, PaymentPartyData> paymentPartyDataCache = new HashMap<>();
    ...
    private Map<BatchKeyGenerator, Map<UniqueBatchKey, List<MapperPayload>>> getMapperPayloadsByUniqueKeyByType(
            List<ConsumerRecord<String, SinkEvent>> messages
    ) {
        return messages.stream()
                .map(ConsumerRecord::value)
                .map(sinkEvent -> Map.entry(sinkEvent.getEvent(), paymentMachineEventParser.parse(sinkEvent.getEvent())))
                ...
                .map(
                        typeEntry -> {
                            Map<UniqueBatchKey, List<MapperPayload>> mapperPayloadsByUniqueKey = typeEntry.getValue().stream()
